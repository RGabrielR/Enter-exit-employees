...
describe('registerEntry', () => {
  ...
  it('throws if already inside without exit', async () => {
    ...
    await expect(
      registerEntry('6869439f15a32310efc3097e', new Date('2025-07-05T11:00:00Z'))
    ).rejects.toThrow('El empleado ya está dentro de la compañía.');
  });
});

describe('registerExit', () => {
  it('throws if there is no prior entry', async () => {
    await expect(registerExit('6869439f15a32310efc3097e', new Date())).rejects.toThrow(
      'No se encontró una entrada previa.'
    );
  });
